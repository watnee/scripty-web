...
(function() {
    ...
    function positionPopup(textarea, start, end) {
        ...
        el.style.minWidth = Math.max(180, Math.round(rect.width * 0.35)) + 'px';
        void end;
    }

    // Small inline line icons (theme-aware via currentColor) for the action rows.
    function actionIcon(name) {
        if (name === 'ignore') {
            // plus-in-circle — add the flagged word to your dictionary
    ...
    // Keep the popup within the viewport after it is shown (avoids the menu
    // being clipped at the right or bottom edge).
    ...
    function showSuggestions(textarea, token) {
        ...
        parts.push(
            '<li role="option" class="scripty-spell-action scripty-spell-ignore" data-action="ignore" data-index="' +
            suggestions.length + '">' + actionIcon('ignore') +
            '<span class="scripty-spell-action-label">Add to dictionary</span></li>'
        );
        parts.push(
            '<li role="option" class="scripty-spell-action scripty-spell-delete" data-action="delete" data-index="' +
            (suggestions.length + 1) + '">' + actionIcon('delete') +
    ...
    function openSuggestionsForTextarea(textarea, pendingClick) {
        ...
        loadDictionary().then(function() {
            ...
            /* dictionary unavailable — nothing to suggest */
        });
    }
